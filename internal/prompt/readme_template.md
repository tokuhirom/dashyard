...
|------|---------|----------------------|
| `README.md` | This guide | No (write-once) |
| `config.yaml` | Dashyard server configuration | No (write-once) |
| `prompt.md` | LLM prompt with guidelines and format reference | No (write-once) |
...
2. **Feed to an LLM** — Pass `prompt.md` and `prompt-metrics.md` to an LLM and ask it to generate dashboards.
...
`prompt.md` is yours to edit freely — it won't be overwritten on subsequent runs. You can customize it to:
